import {
  ...
  u,
  un
} from "./chunk-ARAB6HEI.js";
import "./chunk-G6M2IF7N.js";
import {
  __publicField
...
function Me(e, t) {
  return ht(e.states, function(r, n) {
    if (r) {
      ...
      if (o)
        return { current: o, states: Me(r, o) };
    }
  });
}
function Re(e, t) {
...
function Bt(e, t) {
  if (e)
    return T(e) ? { type: Jt, name: e, predicate: t ? t[e] : void 0 } : A(e) ? { type: Jt, name: e.name, predicate: e } : e;
}
function Ce(e) {
  try {
...
function Qt(e, t, r, n, o) {
  ...
  if (t.type === Jt)
    return ((i == null ? void 0 : i[t.name]) || t.predicate)(r, n.data, a);
...
function mt(e, t) {
  ...
  if (T(e) || typeof e == "number") {
    ...
    r = { type: e.name || e.toString(), exec: e };
  else {
    var n = Kt(e.type, t);
    if (A(n))
      r = v(v({}, e), { exec: n });
...
function hr(e, t, r, n) {
  ...
  if (T(e.delay)) {
    ...
    a = A(e.delay) ? e.delay(t, r.data, o) : e.delay;
      ...
      default:
        ...
        if (i)
          ...
          var Ot = f.length - 1, ee = v(v({}, F), { exec: function(nt) {
            for (var tt = [], G = 1; G < arguments.length; G++)
              tt[G - 1] = arguments[G];
...
function wt(e, t) {
  ...
  try {
    for (var h = b(c), y = h.next(); !y.done; y = h.next())
      for (var p = y.value, S = p.parent; S && !c.has(S); )
        c.add(S), S = S.parent;
  } catch (O) {
    r = { error: O };
  } finally {
  ...
  try {
    for (var g = b(c), w = g.next(); !w.done; w = g.next()) {
      ...
      if (p.type === "compound" && (!m.get(p) || !m.get(p).length))
        d.get(p) ? d.get(p).forEach(function(U) {
          return c.add(U);
  ...
  try {
    for (var P = b(c), D = P.next(); !D.done; D = P.next())
      for (var p = D.value, S = p.parent; S && !c.has(S); )
        c.add(S), S = S.parent;
  } catch (O) {
    u2 = { error: O };
  } finally {
...
var J = function() {
  ...
  return e.from = function(t, r) {
    if (t instanceof e)
      return t.context !== r ? new e({ value: t.value, context: r, _event: t._event, _sessionid: null, historyValue: t.historyValue, history: t.history, actions: [], activities: t.activities, meta: {}, events: [], configuration: [], transitions: [], children: {} }) : t;
    var n = Z;
    ...
    var n = this;
    if (t === void 0 && (t = this.value), r === void 0 && (r = "."), T(t))
      return [t];
    var o = Object.keys(t);
...
var or = function() {
  function e(t, r) {
    ...
    this.machine = t, this.delayedEventsMap = {}, this.listeners = /* @__PURE__ */ new Set(), this.contextListeners = /* @__PURE__ */ new Set(), this.stopListeners = /* @__PURE__ */ new Set(), this.doneListeners = /* @__PURE__ */ new Set(), this.eventListeners = /* @__PURE__ */ new Set(), this.sendListeners = /* @__PURE__ */ new Set(), this.initialized = false, this.status = X.NotStarted, this.children = /* @__PURE__ */ new Map(), this.forwardTo = /* @__PURE__ */ new Set(), this._outgoingQueue = [], this.init = this.start, this.send = function(l, d) {
      if (H(l))
        return n.batch(l), n.state;
      var c = $(vt(l, d));
      if (n.status === X.Stopped)
        return n.state;
      if (n.status !== X.Running && !n.options.deferEvents)
        throw new Error('Event "'.concat(c.name, '" was sent to uninitialized service "').concat(n.machine.id, `". Make sure .start() is called for this service, or set { deferEvents: true } in the service options.
Event: `).concat(JSON.stringify(c.data)));
      ...
      if (!y) {
        if (!h)
          throw new Error("Unable to send event to child '".concat(d, "' from service '").concat(n.id, "'."));
        return;
      }
      if ("machine" in y) {
        ...
        !c && n.machine.config.predictableActionArguments ? n._outgoingQueue.push([y, l.data]) : y.send(l.data);
      ...
      switch (l.type) {
        ...
        case it:
          ...
          if (typeof m.delay == "number") {
            ...
            m.to ? n.sendTo(m._event, m.to, c === Z) : n.send(m._event);
          break;
        case Ft:
          n.cancel(l.sendId);
          break;
  ...
  return Object.defineProperty(e.prototype, "initialState", { get: function() {
    ...
    if (t._sessionid = this.sessionId, this._state = t, (!this.machine.config.predictableActionArguments || r === Z) && this.options.execute)
    ...
    }), this.devTools && this.devTools.send(r.data, t), t.event)
      try {
    ...
    if (this.status !== X.Running)
      return ce(t, r);
    if (ue(t))
      return this.spawnPromise(Promise.resolve(t), r);
    if (A(t))
      return this.spawnCallback(t, r);
    if (Qe(t))
      return this.spawnActor(t, r);
    if (Ce(t))
      return this.spawnObservable(t, r);
    if (Q(t))
      return this.spawnMachine(t, v(v({}, n), { id: r }));
    if (je(t))
      return this.spawnBehavior(t, r);
...
`).concat(x.message));
    ...
    if (f) {
      if (!u2.length)
        return { transitions: [f], exitSet: [], configuration: t.value ? [this] : [], source: t, actions: s };
    ...
    var i = $(r), a;
    if (t instanceof J)
      a = n === void 0 ? t : this.resolveState(J.from(t, n));
    else {
    ...
    if (this.strict && !this.events.includes(i.name) && !Ie(i.name))
    ...
    var q = nt;
    if (!y)
      for (we && (q = this.resolveRaisedTransition(q, { type: Ee }, i, o)); F.length; ) {
        var cr = F.shift();
    ...
    if (!n)
      throw new Error("Child state node '#".concat(r, "' does not exist on machine '").concat(this.id, "'"));
    return n;
    ...
    var r, n = this;
    if (!t)
      return this.initialStateValue || Et;
    switch (this.type) {
      case "parallel":
    ...
    if (Object.keys(this.states).length)
      return { current: t || this.initialStateValue, states: se(this.states, function(r, n) {
        if (!t)
          return r.historyValue();
    ...
    if (t !== void 0)
      return t.map(function(n) {
        ...
        if (o && !r.parent)
          return r.getStateNodeByPath(n.slice(1));
        var i = o ? r.key + n : n;
        if (r.parent)
          try {
...
var fr = class {
  ...
  start(t) {
    ...
    if (typeof r > "u")
      throw on(`invalid state machine id ${t}`);
  ...
  _removeEventListeners() {
    ...
    for (let [r, n] of this._domListeners)
      r === "visibilityChange" && this._player.stopPlayOnShow(), (t = this._domElement) == null || t.removeEventListener(r, n), this._domListeners.delete(r);
    for (let [r, n] of this._playerListeners)
      this._player.removeEventListener(r, n), this._playerListeners.delete(r);
  }
  _addEventListeners() {
    this._requiresDomElement();
    let t = (n) => {
    ...
    this._service.subscribe((n) => {
      ...
      if (typeof n.changed > "u" || n.changed) {
        ...
        for (let i of n.nextEvents)
          if (ur.filter((a) => a !== "complete" && a !== "show").includes(i)) {
            let a = r(i);
            ...
            for (let S of h)
              if (typeof d$1[S] < "u") {
                ...
                if (S === ut.after) {
                  ...
                  S === ut.enter ? y[dn(ut, S)] = { target: (o$1 = m == null ? void 0 : m.state) != null ? o$1 : "" } : y[dn(ut, S)] = { target: (i$1 = m == null ? void 0 : m.state) != null ? i$1 : "" };
              }
            s[l] = { entry: () => {
  ...
  _handlePlaySegments(t) {
    if (typeof t.segments == "string")
      this._player.goToAndPlay(t.segments, true);
    else {
  ...
  _handlePlayOnScroll(t) {
    ...
    if (t.segments && typeof t.segments != "string") {
      ...
      this._player.playOnScroll({ threshold: r });
  }
  ...
  _requiresDomElement() {
    if (!this._domElement)
      throw on("Requires a DOM element to attach events.");
  }
};
export {
...
@dotlottie/common/dist/dotlottie-state-machine-manager-2E7RUGJG.js:
  ...
  *)
*/
//# sourceMappingURL=dotlottie-state-machine-manager-2E7RUGJG-WFCHV6NU.js.map
